...
const Books = () => {
  ...
  const handleUpdate = () => {
    ...
    if(isNaN(id)){
      ...
      fetch(`http://34.27.255.215:30000/updateBook?id=${id}&imgUrl=${imgUrl}&title=${title}&description=${description}&author=${author}&price=${price}&rating=${rating}&type=${type}`, {
        method: 'GET',
        headers: {
  ...
  const handleDelete = () => {
    ...
    if(isNaN(id)){
      ...
      fetch(`http://34.27.255.215:30000/deleteBook?id=${id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const getBooks = async (type) => {
    ...
    await  fetch(`http://34.27.255.215:30000/getBooks?type=${type}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        },
